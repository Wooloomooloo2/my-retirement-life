...
2. ~~Reload the ontology~~ **✓ DONE** — ran `tools/reload_ontology.py` against the live store: **1296 → 1360 triples, version now 1.0.9**; verified `rentalProperty`/`rentalYieldRate` + `importSourceApp`/`importSourceRef`/`importedAt` all declared in the live store. (Data graph untouched.)

**Built this session (code-only, on `main`, all verified on isolated copies):**
- **Dashboard estate hero (item 67).** `src/templates/dashboard.html` only — promoted the life-expectancy net worth to a hero card **"Expected Estate Value at Life Expectancy"** (`snapshots.final.total`; already engine-computed) above the Today / Years-to-retirement / At-retirement cards; dropped the redundant "At life expectancy" card (grid 4→3). On Mark's live copy: today £2,240,080 → estate £1,784,686 in 2062 (£0 cash + £1.03M invest + £0.75M assets).
- **ADR-021 rental income as a yield on a linked property (item 68, Accepted, ontology 1.0.9).** New `mrl:rentalProperty`(→PhysicalAsset) + `mrl:rentalYieldRate` on `IncomeSource`; engine derives rent = yield% × the linked property's **start-of-year** value (auto-stops the year after sale; rides the property's appreciation), ignoring the static amount/growth. Income form gets a Property-type-gated "Linked property" picker + "Net annual yield %" with a live first-year estimate; export/restore carry both (schema **0.3.2**). Verified 7/7 on a copy (parity, static vs yield, appreciation-tracking, sale-stop, round-trip).
- **Monte Carlo now counts unfunded as failure (item 69, backlog clear).** `run_monte_carlo` `success_rate` previously keyed off total balance>0 only, so it reported a confident success on the locked-money case (spending unfunded while a below-access-age pension compounds untouched) — the same false "On track" item 61 fixed for the deterministic engine. Now a sim succeeds only if balance stays >0 every year AND `result["first_unfunded_year"] is None`. `projection.py` only. Verified on a copy: Mark's healthy plan 90.5%; a constructed locked-money plan (all investments `drawdownMinAge=200` → unfunded from 2030, £2.58M, but balance never hits 0 / ends £12.7M locked) now scores **0.0%** where the old test would have said ~100%.
- **Disappearing-★ chart bug fully closed (items 70–71, item 56 done).** The retirement-year ★ was dropped by Chart.js autoSkip on every chart's X-axis tick. Fixed across all 5 charts by plotting the marker as a real series point: stacked-area charts (`dashboard.html` net worth, `projection.html` By-Account, `budget.html`) overlay a transparent marker line at the per-year stack total in its own `stack:'_marker'` group (off the stack; filtered from the tooltip footer, and from the budget legend); line charts (`projection.html` Overview Balance line + Monte Carlo P50 line) attach star points to the existing line. Star heights verified == engine totals (dashboard retirement £2.24M/final £1.78M; By-Account £1.71M/£2.20M). `investment_projection.html` already had the pattern. Budget marker rebuilds in `datasetsFor()` so it survives grouping/inflation toggles. Visual render browser-only — confirm on the live Projection/Budget/Dashboard pages.
- **Header "← Dashboard" breadcrumb (item 72).** Mark flagged that drilling from a dashboard tile into a detail page had no intuitive way back (the sidebar Dashboard link re-highlights silently). Added a contextual back-link in the `base.html` header — `← Dashboard / {page title}` on every page except the dashboard itself (gated `active != 'dashboard'`), linking straight to `/`. Reads as a lightweight breadcrumb. Chose explicit link-to-dashboard over `history.back()` for predictability; chose icon+label over a bare arrow for discoverability. (Open tweak offered: make it step back one level on `/…/edit` sub-pages instead of jumping to the dashboard.)
- **MFL import regression re-verified (de-risks the still-pending live smoke-test).** Re-ran all 4 headless verify tools after this session's edits: `reader` 29/29, `mapping` 29/29, `apply` 17/17, `diff` 13/13 (88/88). The import layer is untouched by the income/settings/projection changes. (Note: `verify_mfl_mapping.py` + `verify_mfl_diff.py` crash on a default Windows console because they `print` a `→`; harmless — run with `PYTHONIOENCODING=utf-8`, or swap `→`→`->` in those two tools. Small backlog nicety.)
- **Routing-vs-drawdown trap UI guard (item 54, backlog clear).** The income deposit-account field now shows a soft inline warning (saving still allowed — chosen over auto-fixing priority or hard-blocking) when income is routed to a non-spending account drawn LATER than the effective spending account AND at the drawn-last default (priority ≥ 999). Trigger is **relative** to the spending account, so an all-default-999 setup doesn't nag. `income.py` `_page_context` passes `account_priorities` (engine truth via `load_all_accounts`) + `effective_spending_label` (configured, else engine fallback: first current → first cash → first acct) + `spending_priority`; `income.html` adds the warning + `updateDepositWarning()` JS. Verified: silent on Mark's data (all 999); after setting the spending account to priority 1, it warns for every drawn-last (999) account but NOT the deliberately-early priority-5/6 investments. **NB: on Mark's current undifferentiated data the warning never fires — by design; it activates once he sets a real drawdown order.**
- **MFL import now maps income instead of dropping it (item 73, ADR-020 amendment).** Mark flagged that the import ignored income ("not part of the budget" — but income IS first-class in MRL). `mapping.py`: MFL budget **income** categories → new `ProposedIncome` (name heuristic → `IncomeSourceType_*`: Salary→Employment, Rent→Property, Pension→Retirement, Dividend/Interest/Business, else Other), net + ongoing (no end year), `source_ref="income:<name>"`; `plan.income` + summary + a review-warning. `apply.py`: create `IncomeSource` on first import (provenance) / **refresh only the annual amount** on re-import (preserves user type/end-year/deposit edits — like account-balance refresh, unlike create-only budget lines); `_refresh_income_amount`, `compute_diff`, `_entity_name` updated. `import_mfl.py` apply route: `inc_include`/`inc_amount`/`inc_type` form fields + filter/overlay; preview passes `INCOME_TYPE_LABELS`. `import_review.html`: income card (include / retype / edit annual) + summary count. Verify tools updated for the new counts (public "Jordan Avery" fixture has 1 income "Salary" → first import 20→**21** entities). **Suite 95/95** (reader 29, mapping 32, apply 21, diff 13); review template compiles. Income currency = base (MFL budget is in base), passed like the account import.
- **MFL import: editable interest on cash accounts (item 74).** Mark flagged that cash accounts couldn't get an interest rate on import — Growth was greyed out and `apply.py` hardcoded `interest_rate=0.0`, so imported savings came in at 0%. Added `ProposedAccount.interest_rate`; `import_review.html` cash rows now show an editable **Interest %** input (the "Growth %" column relabelled "Growth / Interest %"), with hidden inputs keeping the `acct_growth`/`acct_dividend`/`acct_interest` arrays index-aligned per row; `import_mfl.py` reads `acct_interest` for cash rows; `apply.py` uses `a.interest_rate` (default 0). MFL stores no cash interest (transaction ledger), so the field starts blank for the user. Verified: suite still 32/21/13; a 3.5% entry persists as `annualInterestRate`, blank → 0.

**STILL PENDING — MFL import live smoke-test (the only open task; ontology reload + save now done):**
- In the live app **Import from MFL** → pick a real `.mfl` (Mark's own, or `tests/fixtures/mfl_public.mfl`) → preview → apply → complete the retirement layer → re-import the same file to confirm the diff (Update/Keep) and no duplicates. **The wizard now also imports income (item 73) and lets you set cash interest (item 74)** — exercise both: confirm income sources land on the Income page typed correctly, and that savings accounts carry the interest you enter. (All import verification so far was headless on isolated stores — not yet driven in the live desktop app.)
...
- **⚠ AMENDED session 15 (items 73–74)** — the Phase-2/3 descriptions above are the original ADR-020. Since then: MFL budget **income** → `IncomeSource` (no longer skipped), and **cash interest** is editable in the wizard. So the **verify counts moved**: mapping **29→32**, apply **17→21** (public fixture's "Salary" makes first import 20→**21** entities); diff still 13. New `ProposedIncome` + `ProposedAccount.interest_rate`; income refreshes the annual amount on re-import (like a balance), cash interest is create-time. Full suite **95/95**. See the session-15 bullets above for the file-level detail.

**MFL facts** (from inspecting `/Users/markhall/Documents/GitHub/my-financial-life`): transaction-ledger **SQLite** app, **no JSON export**; balances **derived** (`opening_balance + Σ txn.amount` in pence; investments = cash leg + Σ net_qty×price×price_multiplier — `lot`/`valuation` tables empty in practice, positions replayed from txns); MFL `account.family` = cash/credit/investment/loan/property/vehicle; budget categories roll up to child-of-kind-root (Housing, Groceries…). Mapping: cash/investment→accounts, property/vehicle→physical assets, loan→`BudgetLineType_Loan` line, **budget income→`IncomeSource` (item 73)**, credit→skipped; investment growth rates flagged for the user (MFL has no forecasts), **cash interest editable in the wizard (item 74)**.
...
**✓ RESOLVED 2026-06-28 (was the £5.22M-vs-£2.84M data question):** those were two point-in-time states of one **unsaved, drifting** session, not a bug. `active_scenario.json` = `{"name":"Mark Baseline","saved":false}`; the live data graph has diverged from `Mark Baseline.json` (live = 14 accts / £2.24M net worth today / £2.20M spendable-final; saved May-24 file = 12 accts / £1.58M). Mechanism: on-screen figure = live graph; a disk copy = whatever the store last held; they were never the same scenario. **Now fully resolved** — the session was saved headlessly as `Mark Baseline 2026-06.json` and `active_scenario.json` flipped to `saved:true` (see resume block), so the data graph is safe to overwrite/load against again.
