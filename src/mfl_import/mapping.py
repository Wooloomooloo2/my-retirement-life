...
- budget categories arrive already rolled up to top level (reader); income
  categories are not turned into spending lines.
...
def build_plan(snapshot: MflSnapshot, current_year: Optional[int] = None) -> ImportPlan:
    ...
    # Budget: expense categories → spending lines; income categories are not
    # spending and are surfaced as skipped (income is set up in MRL directly).
    if snapshot.budget:
        for c in snapshot.budget.categories:
            if c.kind == "income":
                plan.skipped.append(SkippedItem(
                    f"Budget income: {c.name}",
                    "Income isn't part of the MRL budget — add it under Income."))
                continue
